...
\title[Context-aware Dual-Task Learning for Program Repair]{Context-aware Dual-Task Learning for Program Repair}
...
code context. Despite successes, the state-of-the-art DL-based
APR approaches are still limited in integrating context learning and
code-transformation learning, leading to their ineffectiveness in
fixing context-dependent bugs. Moreover, in those approaches, the
cascading architecture from a code context-learning model (CCL) to a
code-transformation learning model (CTL) possibly leads to the
confounding inaccuracies.
%
In this work, we introduce {\tool}, a context-aware dual-task learning
