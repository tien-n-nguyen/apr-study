...
\title[Context-aware Dual Learning for Automated Program Repair of Context-Dependent Bugs]{Context-aware Dual Learning for Automated Program Repair\\ of Context-Dependent Bugs}
...
those DL-based approaches in learning to auto-fix {\em
...
unit to exploit the duality between CCL and CTL.
...
compared {\tool} against several state-of-the-art pattern-based and
DL-based APR tools. Our results show that {\tool} is able to fix {\bf
XX\%} and {\bf XX\%} more bugs than the best performance pattern-based
and DL-based baseline models, in which several of them are
context-dependent bugs. {\tool} detected {\bf XX} unique bugs that all
the other DL-based APR models missed. Importantly, {\tool} is
automated and data-driven, and does not require bug-fixing patterns.
