...
\title{{\tool}: Context-aware Dual-Task Learning for Automated Program Repair}
...
depend on the code context. Despite successes, the
state-of-the-art Deep Learning (DL)-based APR approaches are still
limited in {\em integrating context learning and code-transformation
  learning}, leading to their ineffectiveness in fixing {\em
  context-dependent bugs}. Moreover, in those approaches, the
cascading architecture from a code context-learning model (CCL) to a
code-transformation learning model (CTL) possibly leads to confounding
inaccuracies.
%
