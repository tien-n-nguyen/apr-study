...
\begin{table}[t]
  \caption{RQ3. Overlapping Analysis. The number in gray: the unique bugs that a model fixed and the other missed.}
...
Fig.~\ref{example_codit} shows an example that was correctly fixed
...
\code{abs}.
%As shown in Fig.~\ref{tree-change}, the structure of the
...
%        \vspace{-9pt}
%	\caption{AST Structural Changes for the Fix in Fig.~\ref{example_3}}
%	\label{tree-change}
%\end{figure}
...
CoCoNuT did not fix correctly the code in Fig.~\ref{example_3}.~It
...
Fig.~\ref{example_3}. It fixed~the line 6 into \code{return
