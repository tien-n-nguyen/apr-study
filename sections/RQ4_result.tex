...
\subsection{\bf RQ3. Impact of Context Learning \& Dual Learning}
\label{rq4:sec}
...
In Table~\ref{fig:rq4_results}, the top-$K$ values of the
\code{Cascading} model (CCL $\rightarrow$ CTL) are 21.5\%, 18.6\%, and
...
between the results from {\tool} and \code{Cascading}. {\tool} fixes {\bf 54} bugs that the cascading model missed and
the \code{Cascading} model fixed only {\bf 17} bugs that {\tool}
...
{\em D.2.1. When more contexts are learned correctly by {\tool} than
  by \code{Cascading} model, does that lead to better bug-fixing?}
...
the \code{Cascading} model and {\tool} against those subtrees in the
...
otherwise. Next, we considered {\em the bugs for which the output of
  ...
  \code{Cascading} model was incorrect}. Among these, the output
of CTL, the actual predicted patch, leads to more correct bug fixes
by {\tool} ({\bf 134}) than by the \code{Cascading} model ({\bf
...
To answer D.2.2., we analyzed {\bf 54} bugs that were fixed by {\tool}
and missed by the \code{Cascading} model. Among them, we found {\bf
...
the output contexts of CCL in the \code{Cascading} model match with
