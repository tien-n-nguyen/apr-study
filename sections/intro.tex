...
DL-based APR approaches. Our results show that {\tool} is able to fix {\bf
  XX\%} and {\bf XX\%} more bugs than the best performance
pattern-based models. {\tool} fixed {\bf XX\%} and {\bf XX\%} more
bugs than the best performance DL-based baselines, in which several of
them are context-dependent bugs. {\tool} detected {\bf XX} unique bugs
that all the other DL-based APR models missed, while {\tool} missed only
{\bf XX} bugs that were fixed by the others.
