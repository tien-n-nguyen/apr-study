...
{\bf XX} more bugs than the best performance DL-based models,
%{\tool} fixed {\bf XX\%}, and {\bf XX\%} more bugs than the best
%performance DL-based baselines,
in which several of them are context-dependent bugs. {\tool} detected
{\bf XX} unique bugs that all the other DL-based APR models missed,
while {\tool} missed only {\bf XX} bugs that were fixed by the others.
In the large datasets Bugs.jar and BigFix, {\tool} can fix {\bf XX\%}
and {\bf XX\%} more bugs than the best performance DL-based models.
