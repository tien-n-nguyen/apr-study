...
to create a new \code{FileInputStream}.  Those context-aware APR models
...
To address that issue, DLFix~\cite{icse20} makes a clear boundary of
...
\code{fopen} as in \code{FILE *fd = fopen (fname, ``rb'');}, then the
succeeding bug-fixing code is more likely to be \code{if (fd != null)}
or \code{if (fd == null)}. However, if the preceding code contains
...
\code{if (n == size)}, rather than a null check (\code{if (n !=
  null)}).  On the other hand, if the bug-fixing transformation is a
change from \code{if (fd == null)} to \code{if (fd != null)}, then the
