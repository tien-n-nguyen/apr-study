...
In addition, other DL-based approaches implicitly learn the
{\em transformation rules} from a buggy code to a correct one
accordingly to the surrounding {\em context} of the
...
Those approaches show that {\em learning bug-fixing changes might be
  dependent on {\em code context}}. For example, in C~code, if the
preceding context contains \code{fopen} as in \code{FILE *fd = fopen
  (fname, ``rb'');}, the succeeding bug-fixing code is more likely to
be \code{if (fd != null)} or \code{if (fd == null)}. However, if the
preceding context contains \code{fread} as in \code{n} \code{=}
\code{fread(buffer,1,size,fd);}, the succeeding bug-fix is more likely
to be \code{if (n} \code{!=} \code{size)} or \code{if (n} \code{==}
\code{size)}, rather than a null check \code{if (n} \code{!=}
\code{null)}. Those context-aware models have achieved better APR
...
They extract contextual features to be fed into a DL
