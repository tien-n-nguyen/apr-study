...
Those approaches show that {\em learning bug-fixing changes needs to
  be dependent on {\bf code context}}. For example, in
...
$\rightarrow$ CTL creates a confounding effect from the inaccuracy of
the learning of the context to the learning of the bug-fixing
transformations. Moreover, the equally important impact from CTL to
...
that {\bf {\em explicitly models the mutual impact of context learning
    and code transformation learning}} and vice versa. We train the
two models simultaneously with soft-sharing their parameters to
...
limitations of the existing DL-based APR approaches: 1) it models both
directions of the mutual impacts of CCL and CTL; 2) it overcomes the
confounding effect in the existing cascading architecture.
...
1) Leveraging dual-task learning to propagate the mutual impacts
of context learning and transformation learning in both directions.

2) Dual-task learning avoids confounding inaccuracies.
...
published~\cite{CDFix2022}).  {\bf Advancing DL-based APR approaches
  with dual-task learning for CCL and CTL}: we evaluated {\tool} against the
DL-based models to show that it can fix more bugs than
those approaches.
