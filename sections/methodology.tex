...
%DLFix has key differences with {\tool}.  First, CCL and CTL are
%different.
...
%Second, in DLFix, {\em
%  CCL cascadingly connects to CTL} in which
The vector for the summarized node is used as a {\em weight}
%representing the impact from CCL to CTL. The weight vector is used i
...
%strategy to learn proper code transformations
...
%for fixing multiple statements in the suitable fixing context.
%leverages DLFix to support multi-statement/ multiple-hunk bugs. For
%comparison with the baselines supporting only single-statement bugs,
%we downgrade DEAR by removing the hunk detection component.
...
%In {\tool}, dual-task learning is used between CCL and CTL.
...
%that parses, models, and
%searches source code, as opposed to natural language text, to fix bugs
%automatically.
...
%it was trained on Defects4J after multiple-statement bugs were filtered.
