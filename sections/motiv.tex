...
Second, some other DL-based APR approaches focus {\em only
...
Fourth, some other DL-based approaches have separate representations
for
...
the surrounding context (the method) to be fed into a DL model to
learn the fixing. With this strategy of one model, {\em those
  approaches do not help their models correctly learn the contexts
  given that the code transformations for bug-fixing are known during
  training and could be used to train the models for context
  learning}. For example, during training, the bug-fixing change from
line 8 to line 9 is known. This knowledge could help a model learn
that the key contextual features would lie at the code at lines 6--7
with \code{getIndexOf} and \code{getDataset}.
...
features for a bug fix). We dedicate a separate model for context
learning and leverage the code transformations for bug-fixing, which
are known at the training time in order to improve context learning,
